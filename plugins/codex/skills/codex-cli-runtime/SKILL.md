...
- `--effort`: accepted values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. Support depends on the model: `max` and `ultra` are only accepted by models that advertise them (e.g. `gpt-5.6-sol` and `gpt-5.6-terra` support both; `gpt-5.6-luna` stops at `max`).
