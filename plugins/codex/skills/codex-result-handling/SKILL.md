...
- For `codex-relay:codex-rescue`, do not turn a failed or incomplete Codex run into a Claude-side implementation attempt. Report the failure and stop.
- For `codex-relay:codex-rescue`, if Codex was never successfully invoked, do not generate a substitute answer at all.
...
- If the helper reports that setup or authentication is required, direct the user to `/codex-relay:setup` and do not improvise alternate auth flows.
