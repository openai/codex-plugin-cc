...
test("transfer delegates the current Claude session directly to native import", () => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      { type: "user", cwd: repo, message: { role: "user", content: "/codex-relay:transfer" } }
  ...
  assert.deepEqual(
    ...
    ["Initial request", "Initial answer", "/codex-relay:transfer"]
  );
});
...
test("review rejects focus text because it is native-review only", () => {
  ...
  assert.match(result.stderr, /\/codex-relay:adversarial-review focus on auth/i);
});
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  assert.match(result.stdout, /`\/codex-relay:status review-live`<br>`\/codex-relay:cancel review-live`/);
...
test("stop hook logs running tasks to stderr without blocking when the review gate is disabled", () => {
  ...
  assert.match(blocked.stderr, /\/codex-relay:status/i);
  assert.match(blocked.stderr, /\/codex-relay:cancel task-live/i);
});
...
test("stop hook does not block when Codex is unavailable even if the review gate is enabled", () => {
  ...
  assert.match(allowed.stderr, /Run \/codex-relay:setup/i);
});
