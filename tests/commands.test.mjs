...
test("review command uses AskUserQuestion and background Bash while staying review-only", () => {
  ...
  assert.match(source, /review-estimate "\$ARGUMENTS --json"/);
  assert.match(source, /`vcs`, `target`, `fileCount`, `diffBytes`, `isEmpty`, and `recommendedMode`/);
  assert.match(source, /Do not run Git or Arc commands directly/i);
  assert.doesNotMatch(source, /git status --short|git diff --shortstat/);
  assert.doesNotMatch(source, /Bash\(git:\*\)/);
  assert.match(source, /The companion script parses `--wait` and `--background`/i);
  assert.match(source, /Claude Code's `Bash\(..., run_in_background: true\)` is what actually detaches the run/i);
  assert.match(source, /recommend background and run the review rather than declaring that there is nothing to review/i);
