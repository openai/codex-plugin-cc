...
test("rescue command absorbs continue semantics", () => {
  ...
  assert.match(rescue, /execution controls, not natural-language task text/i);
  ...
  assert.match(rescue, /subagent is transport-only/i);
  ...
  assert.match(rescue, /return.*stdout.*exactly as-is/i);
  assert.match(rescue, /Leave `--resume` and `--fresh` in the forwarded request/i);
  assert.match(agent, /--resume/);
  assert.match(agent, /--fresh/);
  assert.match(agent, /thin forwarding wrapper/i);
  assert.match(agent, /Never set `run_in_background` on any Bash call/i);
  ...
  assert.match(agent, /foreground task or initial background launch fails, return nothing/i);
  assert.match(agent, /gpt-5-4-prompting/);
  assert.match(agent, /only to tighten the user's request into a better Codex prompt/i);
  assert.match(agent, /Do not use that skill to inspect the repository, reason through the problem yourself, draft a solution, or do any independent work/i);
  assert.match(runtimeSkill, /foreground rescues use one attached `task` call/i);
  assert.match(runtimeSkill, /background rescues use a detached task plus bounded status waits/i);
  assert.match(runtimeSkill, /Do not call `setup`, `review`, or `adversarial-review`/i);
  ...
  assert.match(runtimeSkill, /Do not treat `--background` or `--wait` as natural-language task text/i);
